	...
	//	sqlite_check(conn, dbadr, "foreign_keys", schema);
		ereport(WARNING,
				(errcode(ERRCODE_FDW_UNABLE_TO_CREATE_EXECUTION),
				 errmsg("Foreign keys check still not implemented, only full check was done")));
	}
}
...
/*
 * attach_sqlite_db
 *		Attach SQLite database to a connection with given alias
 */
static int
...
sqlite_fdw_db_attach(PG_FUNCTION_ARGS)
{
	...
	if (!superuser_arg(userid) && userid != server->owner)
	{
		ereport(ERROR,
			(errcode(ERRCODE_FDW_UNABLE_TO_ESTABLISH_CONNECTION),
			 errmsg("Only supersuser or owner of the foreign server can attach a SQLite schema!")));
	}
...
sqlite_fdw_db_detach(PG_FUNCTION_ARGS)
{
	...
	if (!superuser_arg(userid) && userid != server->owner)
	{
		ereport(ERROR,
			(errcode(ERRCODE_FDW_UNABLE_TO_ESTABLISH_CONNECTION),
			 errmsg("Only supersuser or owner of the foreign server can detach a SQLite schema!")));
	}
 ...
 *		Returns application id code - integer file subtype of database.
 *		This could means like OGC GeoPackage file, MBTiles tileset, TeXnicard file etc.
 */
Datum
