...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw
OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'delete');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
SELECT * FROM test_pragma_jms;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'truncate');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
SELECT * FROM test_pragma_jms;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'persist');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
SELECT * FROM test_pragma_jms;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'memory');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
SELECT * FROM test_pragma_jms;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'wal');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
SELECT * FROM test_pragma_jms;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'off');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
SELECT * FROM test_pragma_jms;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'WrongValue');
--Testcase 9: tests for synchronous 'off'
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', synchronous 'off');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', synchronous 'normal');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', synchronous 'full');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', synchronous 'extra');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', journal_mode 'wal', synchronous 'normal');
CREATE FOREIGN TABLE test_pragma_jms (
  attr int
) SERVER server;
...
CREATE SERVER server FOREIGN DATA WRAPPER sqlite_fdw 
   	OPTIONS (database '/tmp/sqlite_fdw_test/core.db', synchronous 'WrongValue');
